...
void drawPointers(float radius)
{
   ...
   for (int i=0; i < 360; i++)
   {
      ...
      glVertex2f(ptr_x + cos(degInRad)*radius, ptr_y + sin(degInRad)*radius);
   }
   ...
   glEnd();
    ...
    {
       float degInRad = i*DEG2RAD;
       glVertex2f(640 + ptr_x + cos(degInRad)*radius, ptr_y + sin(degInRad)*radius);
    }
